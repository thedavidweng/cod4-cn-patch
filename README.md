...
| 特性 | 原版 install.bat (2009) | 本版 (modern) |
|---|---|---|
| 运行平台 | 仅限 Windows | **Windows / Linux / SteamOS / macOS** |
| 依赖 | 3 个未知二进制 EXE (decompressor/replace/compressor) | **纯 Python 3 标准库** |
| 安装方式 | 破坏性覆盖，恢复困难 | **原子操作，自动备份，一键回滚** |
| 推广软件 | 强制安装 Garena 竞舞台 | **无附带推广** |
| 编码 | GBK 批处理，现代终端乱码 | **UTF-8，跨平台正常显示** |
| 防病毒 | 老旧 EXE 常被误报 | **开源脚本，零误报** |
| 启动图 | 强制替换 BMP | **手动选择是否替换，自动匹配格式（BMP/PNG）** |
